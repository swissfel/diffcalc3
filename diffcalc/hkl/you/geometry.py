...
class FourCircle_Ver(YouGeometry):
    """For a diffractometer with angles:
          mu, delta, nu, phi
    """
    def __init__(self):
        YouGeometry.__init__(self, 'fourc', {'eta': 0, 'chi': 0})

    def physical_angles_to_internal_position(self, physical_angle_tuple):
        # mu, delta, nu, eta, chi, phi
        mu, delta, nu, phi = physical_angle_tuple
        return YouPosition(mu, delta, nu, 0, 0, phi)

    def internal_position_to_physical_angles(self, internal_position):
        mu, delta, nu, _, _, phi = internal_position.totuple()
        return mu, delta, nu, phi
